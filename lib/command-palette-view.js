...
export default class CommandPaletteView {
  ...
  async show () {
    if (!this.panel) {
      this.panel = soldat.workspace.addModalPanel({item: this.selectListView})
    }
    ...
    this.activeElement = (document.activeElement === document.body) ? soldat.views.getView(soldat.workspace) : document.activeElement
    this.keyBindingsForActiveElement = soldat.keymaps.findKeyBindings({target: this.activeElement})
    this.commandsForActiveElement = soldat.commands.findCommands({target: this.activeElement})
