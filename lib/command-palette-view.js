...
import SelectListView from "atom-select-list"; // TODO: to be updated
...
export default class CommandPaletteView {
    ...
    this.selectListView = new SelectListView({
      ...
      elementForItem: ({ name, displayName }) => {
        ...
        const matches = this.useAlternateScoring
          ? fuzzaldrinPlus.match(displayName, query)
          : fuzzaldrin.match(displayName, query);
        let matchedChars = [];
        let lastIndex = 0;
  ...
  async show() {
    if (!this.panel) {
      this.panel = soldat.workspace.addModalPanel({
    ...
    this.activeElement =
      document.activeElement === document.body
        ? soldat.views.getView(soldat.workspace)
        : document.activeElement;
    this.keyBindingsForActiveElement = soldat.keymaps.findKeyBindings({
      target: this.activeElement
    });
    this.commandsForActiveElement = soldat.commands.findCommands({
      target: this.activeElement
    });
    this.commandsForActiveElement.sort((a, b) =>
    ...
    if (props.hasOwnProperty("useAlternateScoring")) {
      ...
      if (this.useAlternateScoring) {
        await this.selectListView.update({
          filter: (items, query) => {
            return query
              ? fuzzaldrinPlus.filter(items, query, { key: "displayName" })
              : items;
          }
        });
      } else {
