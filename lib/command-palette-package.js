...
class CommandPalettePackage {
  activate() {
    ...
    this.disposables.add(
      soldat.commands.add("soldat-workspace", "command-palette:toggle", () => {
        this.commandPaletteView.toggle();
      })
    );
    this.disposables.add(
      soldat.config.observe("command-palette.useAlternateScoring", newValue => {
        this.commandPaletteView.update({ useAlternateScoring: newValue });
      })
    );
    this.disposables.add(
      soldat.config.observe("command-palette.preserveLastSearch", newValue => {
